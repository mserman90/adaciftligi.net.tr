          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[11px] font-mono tracking-wider uppercase text-[#E9D9A8] font-semibold flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /> {lang === 'en' ? 'Authorized Access' : 'Yetkili Erişimi'}
              </span>
              <h2 className="text-lg font-bold text-white tracking-tight">
                {lang === 'en' ? 'Ada Farm Admin Portal' : 'Ada Çiftliği Yönetici Girişi'}
              </h2>
            </div>
          </div>
          <p className="text-xs text-[#E6EBDD] mt-2 opacity-90 leading-relaxed">
            {lang === 'en'
              ? 'Verify your identity to access ration formulation, herd zootechnical planning, and economic analysis tools.'
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              >
                ...
                {lang === 'en' ? 'Sample Credentials' : 'Örnek Giriş Bilgileri'}
              </button>
            </div>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-stone-400">
              ...
              <input
                ...
                required
                placeholder={lang === 'en' ? 'Password' : 'Şifreniz'}
          ...
          <div className="p-3 bg-stone-100 rounded-xl border border-stone-200 text-stone-600 text-xs flex items-center justify-between">
            <span className="text-[11px]">
              {lang === 'en' ? 'Default Login:' : 'Varsayılan Giriş:'} <strong>admin</strong> / <strong>ada2024</strong>
            </span>
            <button
              type="button"
              onClick={handleQuickFill}
          ...
          <div className="pt-2">
            ...
            >
              <span>
                {isSubmitting
                  ? (lang === 'en' ? 'Verifying...' : 'Doğrulanıyor...')
                  : (lang === 'en' ? 'Sign In & Go to Ration Platform' : 'Giriş Yap ve Rasyon Sistemine Geç')}
              </span>
          ...
          <div className="text-center pt-1">
            ...
            >
              {lang === 'en' ? 'Return to Farm Website' : 'Çiftlik Tanıtım Sitesine Geri Dön'}
            </button>
          </div>
        </form>
      </div>
