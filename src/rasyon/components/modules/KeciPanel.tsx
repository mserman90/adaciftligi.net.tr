...
import { KeciInputs, KeciTip, KeciYavru } from '../../types';
import { ihtiyacKeci } from '../../utils/nutrition';
import { KECI_TIP, KECI_NOT } from '../../data/reproduction';
import { fmt } from '../../utils/formatters';
import { GlossaryText, FarmerTerm } from '../GlossaryText';

interface KeciPanelProps {
  inputs: KeciInputs;
  onChange: (inputs: KeciInputs) => void;
  onToast: (msg: string) => void;
}

export const KeciPanel: React.FC<KeciPanelProps> = ({
  inputs,
  onChange,
  onToast,
}) => {
  ...
  const handleTipChange = (tip: KeciTip) => {
    const kaba = KECI_TIP[tip].kaba;
    onChange({ ...inputs, tip, kabaMin: kaba });
    onToast(`${KECI_TIP[tip].ad} profili uygulandı — kaba yem alt sınırı güncellendi.`);
  };
    ...
    <div className="bg-[#FCFBF6] border border-[#DCD7C4] rounded-xl overflow-hidden grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="p-6 md:p-7 border-b md:border-b-0 md:border-r border-[#DCD7C4]">
        <div className="mb-5">
          ...
          <select
            value={inputs.tip}
            onChange={(e) => handleTipChange(e.target.value as KeciTip)}
          ...
          >
            <option value="oglak">Besi oğlağı</option>
            <option value="idame">Anaç keçi — idame</option>
            <option value="geblik">Anaç keçi — gebelik (son 6 hafta)</option>
            <option value="lakt">Anaç keçi — laktasyon (süt keçisi)</option>
          </select>
        </div>
        ...
        {inputs.tip === 'geblik' && (
          <div className="mb-5">
            ...
            <select
              value={inputs.yavru}
              onChange={(e) => onChange({ ...inputs, yavru: e.target.value as KeciYavru })}
              className="w-full bg-white border border-[#DCD7C4] rounded-md px-3 py-2.5 font-sans text-sm focus:outline-none focus:border-[#2E5B39]"
            >
              <option value="tek">Tek</option>
              <option value="ikiz">İkiz</option>
              <option value="ucuz">Üçüz</option>
            </select>
          </div>
        )}
      ...
      <div className="p-6 md:p-7">
        ...
        <p className="text-xs text-[#6B7160] leading-relaxed mt-4">
          <GlossaryText text={KECI_NOT[inputs.tip]} />
        </p>
      </div>
    </div>
  );
